...
local Settings = dreezelib.Settings
local Flags = dreezelib.Flags

local SetProps, SetChildren, InsertTheme, Create do
  InsertTheme = function(Instance, Type)
    table.insert(dreezelib.Instances, {
      Instance = Instance,
      Type = Type
    })
    return Instance
  ...
  local function Save(file)
    if readfile and isfile and isfile(file) then
      ...
      if type(decode) == "table" then
        if rawget(decode, "UISize") then dreezelib.Save["UISize"] = decode["UISize"] end
        if rawget(decode, "TabSize") then dreezelib.Save["TabSize"] = decode["TabSize"] end
        if rawget(decode, "Theme") and VerifyTheme(decode["Theme"]) then dreezelib.Save["Theme"] = decode["Theme"] end
      end
    end
  end
  
  pcall(Save, "Dreeze V5.json")
end
...
local Connections, Connection = {}, dreezelib.Connection do
...
local function VerifyTheme(Theme)
  for name,_ in pairs(dreezelib.Themes) do
    if name == Theme then
      return true
    end
  end
...
local Theme = dreezelib.Themes[dreezelib.Save.Theme]

local function AddEle(Name, Func)
  dreezelib.Elements[Name] = Func
end

local function Make(Ele, Instance, props, ...)
  local Element = dreezelib.Elements[Ele](Instance, props, ...)
  return Element
end
...
function dreezelib:SetTheme(NewTheme)
  if not VerifyTheme(NewTheme) then return end
  
  dreezelib.Save.Theme = NewTheme
  SaveJson("dreeze library V5.json", dreezelib.Save)
  Theme = dreezelib.Themes[NewTheme]
  
  Comnection:FireConnection("ThemeChanged", NewTheme)
  table.foreach(dreezelib.Instances, function(_,Val)
...
function dreezelib:SetScale(NewScale)
  NewScale = ViewportSize.Y / math.clamp(NewScale, 300, 2000)
  UIScale, ScreenGui.Scale.Scale = NewScale, NewScale
end

function dreezelib:MakeWindow(Configs)
  local WTitle = Configs[1] or Configs.Name or Configs.Title or "dreeze Library V5"
  local WMiniText = Configs[2] or Configs.SubTitle or "by : dreeze9999"
  ...
  local UISizeX, UISizeY = unpack(dreezelib.Save.UISize)
  ...
  local MainScroll = InsertTheme(Create("ScrollingFrame", Components, {
    Size = UDim2.new(0, dreezelib.Save.TabSize, 1, -TopBar.Size.Y.Offset),
  ...
  ConnectSave(ControlSize1, function()
    if not Minimized then
      dreezelib.Save.UISize = {MainFrame.Size.X.Offset, MainFrame.Size.Y.Offset}
      SaveJson("dreeze library V5.json", dreezelib.Save)
    end
  end)
  
  ConnectSave(ControlSize2, function()
    dreezelib.Save.TabSize = MainScroll.Size.X.Offset
    SaveJson("dreeze library V5.json", dreezelib.Save)
  end)
  ...
  function Window:SelectTab(TabSelect)
    if type(TabSelect) == "number" then
      dreezelib.Tabs[TabSelect].func:Enable()
    else
      for _,Tab in pairs(dreezelib.Tabs) do
  ...
  function Window:MakeTab(paste, Configs)
    ...
    local function Tabs()
      ...
      table.foreach(dreezelib.Tabs, function(_,Tab)
    ...
    FirstTab = true
    local Tab = {}
    table.insert(dreezelib.Tabs, {TabInfo = {Name = TName, Icon = TIcon}, func = Tab, Cont = Container})
    Tab.Cont = Container
    ...
    function Tab:AddSection(Configs)
      ...
      local Section = {}
      table.insert(dreezelib.Options, {type = "Section", Name = SectionName, func = Section})
